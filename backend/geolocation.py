...
def get_ip_geolocation(ip: str) -> dict[str, Any]:
    ...
    if not token:
        ...
        try:
            fallback_url = f"http://ip-api.com/json/{clean_ip}"
            req = urllib.request.Request(fallback_url, headers={"User-Agent": "GmailGuard/1.0", "Accept": "application/json"})
            ...
            pass  # If fallback fails, return the default unavailable below

        result = {
            "ip": clean_ip,
            "status": "unavailable",
            "reason": "IPinfo API token is not configured and fallback failed",
